...
"""
#TODO change those values to match taht of the AK60-1.1 KV140
Servo_Params_Serial = {
    'AK60-6': {
...
PARAMETER_FLAGS = {
    # parameter       : flag
    'MOSFET_TEMP'     : 1, # 2 bytes
    'MOTOR_TEMP'      : 1 << 2, # 4 bytes
    'OUTPUT_CURRENT'  : 1 << 3, # 4 bytes
    'INPUT_CURRENT'   : 1 << 4, # 4 bytes
    'D_CURRENT'       : 1 << 5, # 4 bytes
    'Q_CURRENT'       : 1 << 6, # 4 bytes
    'DUTY_CYCLE'      : 1 << 7, # 2 bytes
    'MOTOR_SPEED'     : 1 << 8, # 4 bytes
    'INPUT_VOLTAGE'   : 1 << 9, # 2 bytes
    'MOTOR_ERROR_FLAG': 1 << 16,# 1 byte
    'MOTOR_POSITION'  : 1 << 17,# 4 bytes
    'MOTOR_ID'        : 1 << 18 # 1 byte
}
"""
...
# The buffer_append methods are used to split variables that span more than a byte into multiple bytes so that they can be sent serially
...
def create_packet(data):
    ...
    if (DL > 256):
        ...
        packet.append(0x02) # append frame header
        packet.append(DL) # append data length
        packet += data # data payload
        crc = crc16(data, DL) # CRC (checksum)
        packet.append(np.uint8(crc >> 8)) # upper 8 bits
        packet.append(np.uint8(crc & 0xFF)) # lower 8 bits
        packet.append(0x03) # frame closing
    return packet
...
class TMotorManager_servo_serial():
    ...
    """
    def __init__(self, port = '/dev/ttyUSB0', baud=921600 , motor_params=Servo_Params_Serial['AK60-6'], max_mosfett_temp = 30,):
        """
        ...
        Args:
            ...
            baud: the baud rate to use for connection. Should always be 961200 as far as I can tell. 921600  ????
    ...
    def __exit__(self, etype, value, tb):
        ...
        if self._ser is not None and self._ser.is_open:
            self._ser.write(self.comm_set_duty_cycle(0.0, set_command=False))
            time.sleep(0.05)
    ...
    def poll_values(self, timeout=0.25):
        """
            Send COMM_GET_VALUES and wait for one parsed response.

            Returns:
                servo_serial_motor_state if successful, otherwise None.

        """
        if not self._entered:
            raise RuntimeError("Call this inside a 'with TMotorManager_servo_serial(...) as dev:' block")

        self._rx_event.clear()
        self._send_specific_command(self.comm_get_motor_parameters(set_command=False))
    ...
    def status_dict(self):
        """
        User-friendly snapshot of the latest parsed motor state.
        """
        err = int(self._motor_state.error)

        return {
            "mos_temperature_C": self._motor_state.mos_temperature,
            "motor_temperature_C": self._motor_state.motor_temperature,
            "output_current_A": self._motor_state.output_current,
            "input_current_A": self._motor_state.input_current,
            "id_current_A": self._motor_state.id_current,
            "iq_current_A": self._motor_state.iq_current,
            "duty": self._motor_state.duty,
            "speed_ERPM": self._motor_state.speed,
            "speed_rad_s_est": self.get_output_velocity_radians_per_second(),
            "input_voltage_V": self._motor_state.input_voltage,
            "position_set": self._motor_state.position_set,
            "position_feedback": self._motor_state.position,
            "control_id_or_motor_id": int(self._motor_state.controlID),
            "Vd_V": self._motor_state.Vd,
            "Vq_V": self._motor_state.Vq,
            "error_code": err,
            "error": ERROR_CODES.get(err, f"UNKNOWN_ERROR_{err}"),
    ...
    def print_status(self):
        """
        Pretty-print one fresh COMM_GET_VALUES response.
        """
        st = self.poll_values()
        if st is None:
            print("No COMM_GET_VALUES response received.")
            return

        s = self.status_dict()
        print("---- CubeMars motor status ----")
        print(f"MOS temp:        {s['mos_temperature_C']:.1f} C")
        print(f"Motor temp:      {s['motor_temperature_C']:.1f} C")
        print(f"Input voltage:   {s['input_voltage_V']:.2f} V")
        print(f"Input current:   {s['input_current_A']:.3f} A")
        print(f"Output current:  {s['output_current_A']:.3f} A")
        print(f"Iq current:      {s['iq_current_A']:.3f} A")
        print(f"Id current:      {s['id_current_A']:.3f} A")
        print(f"Duty:            {s['duty']:.4f}")
        print(f"Speed:           {s['speed_ERPM']:.1f} ERPM")
        print(f"Position set:    {s['position_set']:.6f}")
        print(f"Control/Motor ID:{s['control_id_or_motor_id']}")
        print(f"Vd/Vq:           {s['Vd_V']:.3f} / {s['Vq_V']:.3f} V")
        print(f"Error:           {s['error_code']} ({s['error']})")
        if s["last_print"]:
            print(f"Last text print: {s['last_print']!r}")
    ...
    def set_motor_current_qaxis_amps(self, curr):
        ...
        if np.abs(curr) >= self.motor_params["Curr_max"]:
            raise RuntimeError(
                ...
                + str(self.motor_params["Curr_max"]) + " A!"
            )
